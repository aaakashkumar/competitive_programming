...
def color_graph(graph, colors):
    ...
    for current_node in graph:
        ...
        if current_node in current_node.neighbors:
            # O(1) since self.neighbors is a set
            raise Exception("Loop found. A node cannot have a different color than itself")
        
        illegal_colors = set([
                neighbor.color for neighbor in current_node.neighbors if neighbor.color
            ])
        ...
        if legal_color == None:
            raise Exception("No legal coloring found. Are you sure there are D+1 colors available?")
        else:
            current_node.color = legal_color
