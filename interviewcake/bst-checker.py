...
def is_binary_search_tree(root):
    ...
    nodes = [(root, -float('inf'), float('inf'))]  # node, lower_bound, upper_bound
    ...
    while nodes:
        ...
        if node.left:
            ...
            if node.value < upper_bound: temp_upper_bound = node.value
            nodes.append((node.left, lower_bound, temp_upper_bound))
        
        if node.right:
            temp_lower_bound = lower_bound
            if node.value > lower_bound: temp_lower_bound = node.value
            nodes.append((node.right, temp_lower_bound, upper_bound))
